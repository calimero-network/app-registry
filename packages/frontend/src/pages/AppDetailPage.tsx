...
export function AppDetailPage() {
  ...
  const { data: manifest, isLoading: manifestLoading } = useQuery({
    ...
    queryFn: () => appsApi.getAppManifest(pubkey!, appName!, latestVersion!.semver),
          ...
          <div className="card p-6">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm text-gray-900 font-mono">{manifest.app.id}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-500">Supported Chains</label>
              ...
              {manifest.artifacts.map((artifact, index) => (
                <div key={index} className="border border-gray-200 rounded p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-gray-500">{artifact.target}</span>
                  </div>
                  ...
                  {artifact.mirrors && artifact.mirrors.length > 0 && (
                    <div className="mt-2">
                      ...
                      {artifact.mirrors.map((mirror, mirrorIndex) => (
                        <div key={mirrorIndex} className="text-xs text-blue-600 hover:underline">
        ...
        {versionsLoading ? (
          ...
          <div className="space-y-3">
            {versions.map((version) => (
              <VersionRow key={version.semver} version={version} pubkey={pubkey} appName={appName} />
            ))}
          </div>
        )}
      </div>
...
function VersionRow({ version, pubkey, appName }: { version: VersionInfo; pubkey: string; appName: string }) {
  return (
