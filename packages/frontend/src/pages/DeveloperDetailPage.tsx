...
import { ArrowLeft, Users, Package, Globe, Shield, CheckCircle, XCircle } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-6">
        <div className="flex items-start justify-between">
          ...
          <div className="text-right">
            ...
            <div className="text-2xl font-bold text-primary-600">{apps.length}</div>
          </div>
        </div>
      </div>
      ...
      {profile?.proofs && profile.proofs.length > 0 && (
        <div className="card p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Verification Proofs</h2>
          <div className="space-y-3">
            {profile.proofs.map((proof, index) => (
              <div key={index} className="flex items-center justify-between p-3 border border-gray-200 rounded">
                <div>
                  <div className="font-medium text-gray-900">{proof.type}</div>
                  <div className="text-sm text-gray-500 font-mono">{proof.value}</div>
                </div>
