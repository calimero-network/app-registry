...
// ——— Card formatting ———
//
// Every one of these takes a value that is legitimately absent in production
// data: three published bundles have no icon, and `installSize`/`publishedAt`
// are null for everything published before the metadata policy shipped. They
// return null rather than a placeholder string so a caller decides whether to
// render a dash, a skeleton, or nothing at all — a formatter that invents
// "0 bytes" or "Invalid Date" is confidently wrong and looks deliberate.
...
export function formatBytes(bytes?: number | null): string | null {
  ...
  // One decimal below 10 ("1.2 MB"), none above ("200 KB") — the extra digit
  // is noise at three figures and makes card rows ragged.
...
export function formatRelativeDate(iso?: string | null): string | null {
  ...
  // A clock skew or a just-written timestamp can land microseconds in the
  // future; "in 0 seconds" is worse than "just now".
...
/**
 * A stable hue for an app with no icon, derived from its package id so the
 * same app always gets the same tint across renders and machines.
 */
