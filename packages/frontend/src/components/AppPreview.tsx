    ...
    <section data-testid='app-preview' aria-label='Preview'>
      ...
      {assets.length === 0 ? (
        <div
          ...
          className='flex h-44 items-center justify-center rounded-xl border border-dashed border-white/[0.1] bg-white/[0.02]'
        >
    ...
    <div className='group relative h-44 flex-shrink-0'>
      {asset.kind === 'video' ? (
        <video
          ...
          controls
          preload='metadata'
          className='h-44 rounded-xl border border-white/[0.08]'
        />
      ) : (
        <img
          src={asset.url}
          ...
          loading='lazy'
          decoding='async'
          className='h-44 rounded-xl border border-white/[0.08] object-cover'
        />
      )}
      {canEdit && (
        <button
