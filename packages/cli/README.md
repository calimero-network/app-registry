...
`apps`, `developers`, and `attestations` speak the older V1 manifest API through the client library, and default to `http://localhost:8082` rather than the public registry. They are unrelated to the V2 bundle flow above.

```bash
calimero-registry apps list [-n <name>]
calimero-registry apps versions <appId>
calimero-registry apps manifest <appId> <version>
calimero-registry apps submit <manifest-file>

calimero-registry developers get    <pubkey>
calimero-registry developers create <pubkey> <display-name> [-w <url>] [-p <proofs>]

calimero-registry attestations get <pubkey> <name> <version>
```
