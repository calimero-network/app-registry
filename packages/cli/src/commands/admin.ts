...
adminCommand
  ...
  .action(async options => {
    try {
      ...
      console.log(
        `2. Developer runs: ssapp-registry certificate install ${filename}`
      );
      console.log(
        `3. Certificate will be automatically used for all operations`
      );
    } catch (error) {
      console.error(
...
adminCommand
  ...
  .description(
    'Automatically generate certificates for all whitelisted developers'
  )
  .option(
    '-o, --output <path>',
  ...
  .action(async options => {
    try {
      ...
      console.log(
        '🔐 Generating certificates for all whitelisted developers...'
      );
      ...
      console.log(
        '3. Developers install with: ssapp-registry certificate install <file>'
      );
    } catch (error) {
      console.error(
