      ...
      schema: {
        body: {
          ...
          properties: {
            ...
            company: { type: 'string' },
            role: { type: 'string' },
            expiresInDays: { type: 'integer', minimum: 1, maximum: 365, default: 7 }
          }
        },
        response: {
          201: {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        const registryKeysPath = path.join(__dirname, '../../../registry-keys.json');
        ...
        if (fs.existsSync(registryKeysPath)) {
          ...
          fs.writeFileSync(registryKeysPath, JSON.stringify(registryKeys, null, 2));
        }
        ...
        const signedCertificate = signCertificate(certTemplate, registryKeys.privateKey);
        ...
        const { addCertificateToDeveloper } = require('../models/developer-registry');
        addCertificateToDeveloper(invite.email, certificateId, signedCertificate);
        ...
        reply.send({
          message: 'Certificate generated successfully! Save this file and install it using: ssapp-registry certificate install certificate.json',
          ...
          instructions: {
            ...
            step3: 'Install certificate: ssapp-registry certificate install certificate.json',
